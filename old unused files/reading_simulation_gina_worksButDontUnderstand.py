...
#        textsplitbyspace = textsplitbyspace[:1000]
    ...
    # -------------------------------------------------------------------------------------------
        ...
        # ----------------------------------------------------------------------------
            ...
            ### GS added this 11-11
            # MM: recognized_lexicon_np: array w. indices, 1 for wrds with act>thresh., 0 otherwise
            #     then check which already recogn within this fix, make array new_recognized_words with words that now recogn.
            ...
            for word in new_recognized_words: #GS line 786 in old
                my_print('Newly over thresh.: '+lexicon[word] + ', cycle ' +str(amount_of_cycles) +',',str(round(lexicon_word_activity_np[word]/lexicon_thresholds_np[word],3) + ' (ratio crt. activity to threshold)')
                alldata_recognized_append(word) 
                # if m.similarity_based_recognition=true, words are only 'recognized' if word length of word>threshold similar to length word in stim
