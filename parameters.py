...
def return_global_params():
    ...
    Added PSCall for normal text reading in german, even though it is not an exp.
sh
    Returns
    -------
    ...
    task_to_run='EmbeddedWords' #NV: task to run.
    
    short={'french': 'fr', 'german':'de', 'english':'en'} #NV: dictionnary for abbreviations, useful for filenames    
...
class TaskAttributes: 
    ...
    def __init__(self, stim, stimAll, language, stimcycles, is_experiment, 
        ...
        self.totalcycles=self.blankscreen_cycles_begin+self.ncyclesprime+self.stimcycles+ self.blankscreen_cycles_end
...
def return_attributes(task_to_run): #NV: function returns instance of TaskAttributes with corresponding attributes
    if task_to_run == 'EmbeddedWords':
        ...
        stim=pd.read_table('./Stimuli/Flanker_stimuli_all_csv.csv', sep=',') #NV: extra assignments needed for this task
        stim['all']=stim['all'].astype('unicode')
...
def return_task_params(task_attributes):
    ...
    if task_attributes.is_experiment: #NV: if task is an experiment, choose this specific set of params (previously in parameters_exp.py)
        ...
        bigram_to_word_excitation = 1.25 #1.5 # inp. divded by #ngrams, so this param estimates excit per word [diff from paper]
        ...
        max_threshold = 0.8 #1
        wordfreq_p = 0.2 #0.4 # Max prop decrease in thresh for highest-freq wrd [different definition than in papers]
        wordpred_p = 0.2 #0.4 # Currently not used
        
    
        task_params=dict(locals())
        task_params.pop('task_attributes') #NV: is given as input, so would end up in the namespace if not removed. 
        
        return task_params
    
    else: #NV: different set of parameters if task is not an experiment. (previously in parameters.py). This way, one can change params for experiments without interfering in PSC and vice versa
        ...
        task_params=dict(locals())
        task_params.pop('task_attributes') #NV: is given as input, so is a local variable. Would end up in the dictionary if not removed. 
        
        return task_params
    
def return_params():  
    
    global_params=return_global_params() #NV: first, get global, task-independent variables. Values like which task to run, whether we want to run or optimize this task. returns dictionary

    task_attributes=return_attributes(global_params['task_to_run']) #NV: fetch all attributes of the task to run, specified in parameters_exp. Creates an object 

    task_params=return_task_params(task_attributes) #NV: get parameters corresponding to type of given task (different set of parameters for experiment/non-experiment).returns dictionary

    pm=SimpleNamespace(**{**global_params,**task_attributes.__dict__,**task_params}) #put all attributes of separate objects into on pm object
