...
prefixes_FR.rename(columns={"Unnamed: 0": "morpheme"})
suffixes_FR.rename(columns={"Unnamed: 0": "morpheme"})
...
suffixes_FR["morpheme"] = suffixes_FR["morpheme"].map(
    lambda L: L.replace(">", "").replace("[VB]", ""))
suffixes_FR["morpheme"] = suffixes_FR["morpheme"].map(lambda L: L.translate(str.maketrans("","", string.punctuation)))

##TODO: "-" stays in there, for some reason.
...
#old datam (Jarmulowicz, 2002), kept for reference
...
for ix, frame in [prefixes_EN, suffixes_EN, prefixes_FR, suffixes_FR]:
